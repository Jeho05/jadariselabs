...
export default function VideoStudioPage() {
  ...
  const maxDuration = modelInfo?.maxDuration ?? 5;
  const durationOptions = useMemo(() => [3, 5, 6].filter((d) => d <= maxDuration), [maxDuration]);
                ...
                {durationOptions.map((value) => {
                  ...
                  return (
                    ...
                    <button
                      ...
                      className={`p-3 rounded-2xl border-2 transition-all ${duration === value ? 'border-[var(--color-gold)] bg-[var(--color-gold)]/5' : 'border-[var(--color-cream-dark)] bg-white hover:border-[var(--color-gold)]/50'}`}

                    >

                      {duration === value && <IconCheck size={14} className="absolute top-2 right-2 text-[var(--color-gold)]" />}

                      <div className="font-bold">{value}s</div>
